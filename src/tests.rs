...
async fn listen(addr: SocketAddr, host_key: &signature::RsaKeyPair) -> Result<()> {
...
fn read_rsa_testkey() -> Result<(signature::RsaKeyPair, Vec<u8>)> {
    ...
    Ok((key_pair, key_pair.public_key().as_ref().to_vec()))
}

fn spawn_n_peers(n: usize) -> Vec<Peer> {
    let (host_key, peer_key) = read_rsa_testkey().unwrap();
    let mut peers = Vec::new();
    for i in 0..n {
        let peer_addr = (TEST_IP, TEST_PORT + i as u16).into();
        tokio::spawn(async {
            listen(peer_addr, &host_key).await.unwrap();
        });
